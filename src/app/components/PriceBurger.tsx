...
const PriceBurger = ({ price, id, options }: Props) => {
  ...
  const burgerId=Number(params.id)-1
  const item=burgers[(burgerId)]
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="uppercase w-56 bg-red-500 text-white p-3 ring-1 ring-red-500 text-center hover:bg-red-600 rounded-r-full transition-all duration-300">
          ...
          <button className="" onClick={()=>dispatch(add({item,quantity,total}))}>Add To Cart</button>
        </div>
      </div>
    </div>
  );
